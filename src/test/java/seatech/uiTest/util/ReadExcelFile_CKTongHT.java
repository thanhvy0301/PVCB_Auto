...
public class ReadExcelFile_CKTongHT {
    ...
    public ReadExcelFile_CKTongHT(WebDriver driver){
        this.driver =driver;
    ...
    String accountNo_s3;
    String receiveAccount_s3;
    String amount_s3;
    String contentTran_s3;
    String asTotal_s3;
    String asTotalReceive;
    String nameReceiveAccount;
    String nameReceiveAccount_s3;
    String stringAmount;
    String stringAmount_s3;
    String dataExcel;
    String actionExcel;
    ...
    @Step("Test: Step enter Url")
    public void enterUrl(String action){
        driver.get(getDataExcel());
        Log.info("Step: " + action + " | " + "Khởi tạo đường dẫn: " + getDataExcel());
    }
    @Step("Test: Step enter Username")
    public void enterUserName(String action){
        try {
            driver.findElement(By.id("t_userName")).sendKeys(getDataExcel());
            Log.info("Step: " + action + " | " + "Nhập username " + getDataExcel());
        }catch (NoSuchElementException e){
            Log.error("Không tìm thấy username");
        }
    }
    @Step("Test: Step enter Password")
    public void enterPassword(String action) throws InterruptedException {
        try {
            driver.findElement(By.id("t_password")).sendKeys(getDataExcel());
            Log.info("Step: " + action + " | " + "Nhập username " + getDataExcel());
            Thread.sleep(10000);
        }catch (NoSuchElementException e){
    ...
    public void navigateMenu(String action){
        ...
        if (action.equalsIgnoreCase("navigateMenu")) {
            String[] menuItems = getDataExcel().split(",\\s*"); // Split chuỗi dựa trên dấu phẩy và khoảng trắng sau dấu phẩy
    ...
    public void selectTabTrans(String action){
        try {
            WebElement typeTran = driver.findElement(By.xpath("//label[contains(text(),'" + getDataExcel() + "')]"));
            typeTran.click();
    ...
    public  void selAccountNo(String action){
        setAccountNo(null);
        try {
            Select boxAccountNo = new Select(driver.findElement(By.id("selectAccountNo")));
            boxAccountNo.selectByValue(getDataExcel());
            setAccountNo(getDataExcel());
            Log.info("Step: " + action + " | " + "Chọn tk trích nợ: " + getDataExcel());
            Thread.sleep(2000);
        } catch (NoSuchElementException e) {
    ...
    public void selectReceiveAcc_SA(String action){
        try {
            WebElement txbReceiveAccount = driver.findElement(By.id("receiveAccount"));
            txbReceiveAccount.sendKeys(getDataExcel());
            if (txbReceiveAccount.isDisplayed()) {
                Select boxReceiveAccount = new Select(driver.findElement(By.id("receiveAccount")));
                boxReceiveAccount.selectByValue(getDataExcel());
                setReceiveAccount(getDataExcel());
                Log.info("Step: " + action + " | " + "Chọn tk nhận: " + getDataExcel());
                Thread.sleep(2000);
            }
    ...
    public void clickBtnContinueS1_SameAccount(String action){
        try {
            ...
            Log.info("Step: " + action + " | " + "Nhấn tiếp tục để sang bước 2 (Cùng chủ)");
            try {
                if (getDataList.get(0).isEmpty()) {
                ...
                } else if (getDataList.get(1).isEmpty()) {
                    String txtErrReceiveNo = driver.findElement(By.xpath("//table[3]/tbody[1]/tr[1]/td[2]/span[1]/font[1]")).getText();
                    Log.info("Step: " + action + " | " + "Hiển thị thông báo: " + txtErrReceiveNo);
                } else if (getDataList.get(2).isEmpty()) {
                ...
                } else if (getDataList.get(3).isEmpty()) {
    ...
    public void clickBtnContinueS2_SameAccount(String action){
        ...
        try {
            ...
            //String result = txtAccountNo.replace(" VND", "");
            ...
            setStringAmount_s3(asStringAmount);
    ...
    @Step("Test: Step End")
    public void clickToStep3(String action){
        try{
    ...
    //===================================KHÁC CHỦ=======================
    @Step
    public void enterReceiveAccount_DiffAccount(String action){
        try {
            WebElement txbReceive = driver.findElement(By.id("receiveAccount2"));
            txbReceive.clear();
            txbReceive.sendKeys(getDataExcel());
    ...
    @Step
    public void enterAmount_DiffAccount(String action){
        try {
    ...
    @Step
    public void enterContentTrans_DiffAccount(String action){
        try {
            WebElement txbContent_Diff = driver.findElement(By.id("textarea2"));
            txbContent_Diff.clear();
            txbContent_Diff.sendKeys(getDataExcel());
    ...
    @Step("Step: Click button end trans")
    ...
    public void clickChooseTransSaved(String action){
        WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(10));
        try {
            ...
            try {
                WebElement dialogTransSaved = wait.until(ExpectedConditions.visibilityOfElementLocated(
                        By.xpath("//table[1]/tbody[1]/tr[" + getDataExcel() + "]/td[1]/input[1]")));
    ...
    public void clickOkSaveTemplateTrans(String action){
        WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(10));
        try {
            ...
            if (dialogSaveTemplate.isDisplayed()) {
                if (getDataExcel().isEmpty()) {
                    driver.findElement(By.xpath("//button[@title='Close']")).click();
                    Thread.sleep(2000);
                } else {
                    driver.findElement(By.id("templateNameId")).sendKeys(getDataExcel());
                    Log.info("Step: " + action + " | " + "Nhập tên giao dịch mẫu: " + getDataExcel());
                    Thread.sleep(2000);
    ...
    public void chooseSaveReceiveAccount(String action){
        WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(10));
        try {
            ...
            driver.findElement(By.xpath("//table[1]/tbody[1]/tr[" + getDataExcel() + "]/td[1]/input[1]")).click();
            Log.info("Step: " + action + " | " + "Nhấn chọn  mẫu đã lưu: " + getDataExcel());
    ...
    public void selectTransType(String action){
        try {
            Select boxAccountNo = new Select(driver.findElement(By.id("tansferType")));
            if (getDataExcel().equals("Chuyển khoản trong hệ thống")) {
                boxAccountNo.selectByValue("1");
                Log.info("Step: " + action + " | " + "Chọn loại giao dịch: " + getDataExcel());
            } else if (getDataExcel().equals("Chuyển khoản ngoài hệ thống")) {
                boxAccountNo.selectByValue("2");
                Log.info("Step: " + action + " | " + "Chọn loại giao dịch: " + getDataExcel());
            } else if (getDataExcel().equals("Chuyển khoản định kỳ")) {
                boxAccountNo.selectByValue("3");
                Log.info("Step: " + action + " | " + "Chọn loại giao dịch: " + getDataExcel());
            } else if (getDataExcel().equals("Chuyển khoản nhanh 24/7")) {
                boxAccountNo.selectByValue("6");
                Log.info("Step: " + action + " | " + "Chọn loại giao dịch: " + getDataExcel());
            } else if (getDataExcel().equals("Bán Ngoại Tệ cho PVcomBank")) {
    ...
    public void tickTransaction(String action){
        try {
            String[] itemTrans = getDataExcel().split(",\\s*"); // Split chuỗi dựa trên dấu phẩy và khoảng trắng sau dấu phẩy
            ...
            Log.info("Step: " + action + " | " + "Tick chon giao dịch: " + getDataExcel());
            Thread.sleep(2000);
        } catch (NoSuchElementException e) {
    ...
    public void clickChooseSecurityDevice(String action){
        try {
            Select boxAccountNo = new Select(driver.findElement(By.xpath("//select[@id='allCode']")));
            boxAccountNo.selectByVisibleText(getDataExcel());
    ...
    public void enterReasonReject(String action){
        try{
            driver.findElement(By.id("remarks")).sendKeys(getDataExcel());
            Log.info("Step: "+action +" | "+"Nhập nội dung từ chối: "+getDataExcel());
        }
        catch (NoSuchElementException e){
    ...
    public void enterOTP(String action){
        try{
            driver.findElement(By.id("OTP")).sendKeys(getDataExcel());
            Log.info("Step: "+action +" | "+"Nhập mã otp: "+ getDataExcel());
            Thread.sleep(2000);
        }
        catch (NoSuchElementException e){
    ...
    public void showDetailTrans(String action){
        try{
            driver.findElement(By.xpath("//tbody/tr["+getDataExcel()+"]/td[10]/a[1]")).click();
            Log.info("Step: "+action +" | "+"Tick chọn giao dịch để xem chi tiết: "+getDataExcel());
            Thread.sleep(2000);
        }catch (NoSuchElementException e){
    ...
    public void clickBtnShowDetailLimitAmount(String action){
        try{
            driver.findElement(By.xpath("//a[contains(.,'Chi tiết hạn mức')]")).click();
            Log.info("Step: "+action +" | "+"Nhấn nút xem Chi tiết hạn mức: "+getDataExcel());
            Thread.sleep(2000);
        }catch (NoSuchElementException | InterruptedException e){
            Log.error("Không tìm thấy nút xem Chi tiết");
        }
    ...
    public void clickBtnReturn_DetailLimitAmount(String action){
        try{
            driver.findElement(By.id("submit1")).click();
            Log.info("Step: "+action +" | "+"Nhấn nút Quay lại"+getDataExcel());
            Thread.sleep(2000);
        }catch (NoSuchElementException e){
    ...
    @Step
    public void clickChooseNumPage(String action){
        try{
            WebElement btn = driver.findElement(By.id("gotoPageNO_pendingRemittance"));
            btn.sendKeys(getDataExcel());
            Log.info("Nhập số trang: "+getDataExcel());
            driver.findElement(By.xpath("//a[contains(.,'Go')]")).click();
            Log.info("Chuyển đến trang số: "+getDataExcel());
        }
        catch (NoSuchElementException e){
            Log.error("Không tìm thấy số trang: "+getDataExcel());
        }
    }
    public void startExecution(String sheetName) throws InterruptedException {
        ...
        for (int i = 0; i <sheet.getLastRowNum(); i++) {
            ...
            switch (action) {
                case "url":
                    enterUrl(action); break;
                case "enterUsername":
                    try {
                        enterUserName(action);
                    }catch (NoSuchElementException ignored){}
                    break;
                case "enterPassword":
                    try {
                        enterPassword(action);
                    }catch (NoSuchElementException ignored){}
                    break;
                case "clickButtonLogin":
                    ...
                    break;
                case "activeOTPLater":
                    clickVerifyOTPLater(action);
                    break;
                case "navigateMenu":
                    navigateMenu(action);
                    break;
                case "selectTabTypeTran":
                    selectTabTrans(action);
                    break;
                ...
                case "selectReceiveAccount_SameAccount":
                    selectReceiveAcc_SA(action);
                    break;
                case "enterAmount_SameAccount":
                    enterAmount_SameAccount(action);
                    break;
                case "enterContentTran_SameAccount":
                    enterContent_SA(action);
                    break;
                case "tickAgreeCondition_SameAccount":
                    tickAgreeCondition(action);
                    break;
                case "clickBtnContinueS1_SameAccount":
                    clickBtnContinueS1_SameAccount(action);
                    break;
                case "clickBtnContinueS2_SameAccount":
                    clickBtnContinueS2_SameAccount(action);
                    break;
                case "clickButtonEndTran_SameAccount":
                    clickButtonEndTran_SameAccount(action);
                    break;
                case "clickButtonEndTran_OF_SameAccount":
                    clickButtonEndTran_OF_SameAccount(action);
                    break;
                case "clickLinkCondition":
                    clickLinkCondition(action);
                    break;
                case "clickAcceptLogout":
                    clickAcceptLogout(action);
                    break;
                case "saveTrans_SameAccount":
                    saveTrans_SameAccount(action);
                    break;
                case "clickBtnReset_SameAccount":
                    clickBtnReset_SameAccount(action);
                    break;
                case "clickBtnReturn":
                    clickBtnReturn(action);
                    break;
                //===================================KHÁC CHỦ=====================================
                case "enterReceiveAccount_DiffAccount":
                    enterReceiveAccount_DiffAccount(action);
                    break;
                case "enterAmount_DiffAccount":
                    enterAmount_DiffAccount(action);
                    break;
                case "enterContentTrans_DiffAccount":
                    enterContentTrans_DiffAccount(action);
                    break;
                case "tickAgreeCondition_DiffAccount":
                    tickAgreeCondition_DiffAccount(action);
                    break;
                case "clickBtnContinueS1_DiffAccount":
                    clickBtnContinueS1_DiffAccount(action);
                    break;
                ...
                case "clickButtonEndTran_DiffAccount":
                    clickButtonEndTran_DiffAccount(action);
                    break;
                case "clickButtonEndTran_OF_DiffAccount":
                    clickButtonEndTran_OF_DiffAccount(action);
                    break;
                case "saveTrans_DiffAccount":
                    saveTrans_DiffAccount(action);
                    break;
                case "clickBtnReset_DiffAccount":
                    clickBtnReset_DiffAccount(action);
                    break;
                case "clickChooseTransSaved":
                    clickChooseTransSaved(action);
                    break;
               /* case "clickOkSaveTemplateTrans":
                    try{
                        ...
                        Log.info("Step: "+action+" | "+ "Nhấn radio button 'Có' lưu mẫu giao dịch (Khác chủ)");
                        Thread.sleep(3000);
                        Alert showAlert = wait.until(ExpectedConditions.alertIsPresent());
                        Log.info("Đợi Alert hiển thị");
                        if(showAlert!=null){
                            Log.info("Hiển thị alert mẫu giao dịch đã tồn tại");
                            showAlert.accept();
                            Log.info("Nhấn Ok alert");
                            ...
                            Log.info("Không hiển thị alert thông báo mẫu giao dịch đã tồn tại");
                            WebElement dialogSaveTemplate = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("/html[1]/body[1]/div[5]")));
                            if(dialogSaveTemplate.isDisplayed()){
                                driver.findElement(By.id("templateNameId")).sendKeys(data);
                                Log.info("Step: "+action+" | "+ "Nhập tên giao dịch mẫu: "+data);
                        ...
                        Log.error("Không tìm thấy nút Dialog lưu mẫu (Khác chủ)");
                    }
                    break;*/
                case "clickOkSaveTemplateTrans":
                    clickOkSaveTemplateTrans(action);
                    break;
                case "closeSaveTransDialog_DiffAccount":
                    closeSaveTransDialog_DiffAccount(action);
                    break;
                case "clickSaveReceiveAccount":
                    clickSaveReceiveAccount(action);
                    break;
                case "clickNoSaveReceiveAccount":
                    clickNoSaveReceiveAccount(action);
                    break;
                case "chooseSaveReceiveAccount":
                    chooseSaveReceiveAccount(action);
                    break;
                //==============================MÀN HÌNH CHECKER===========================
                case "clickBtnQueryTrans":
                    clickBtnQueryTrans(action);
                    break;
                case "selectTransType":
                    selectTransType(action);
                    break;
                case "tickTransaction":
                    tickTransaction(action);
                    break;
                case "clickChooseSecurityDevice":
                    clickChooseSecurityDevice(action);
                    break;
                case "clickBtnApproveTrans":
                    clickBtnApproveTrans(action);
                    break;
                case "clickBtnApproveTrans_DisplayAlert":
                    clickBtnApproveTrans_DisplayAlert(action);
                    break;
                case "clickSendOtpAgain":
                    clickSendOtpAgain(action);
                    break;
                case "enterReasonReject":
                    enterReasonReject(action);
                    break;
                case "clickContinueS2_Checker":
                    clickContinueS2_Checker(action);
                   break;
                case "enterOTP":
                    enterOTP(action);
                    break;
                case "clickEndTransApprove":
                    clickEndTransApprove(action);
                    break;
                case "clickBtnReject":
                    clickBtnReject(action);
                    break;
                case "showDetailTrans":
                    showDetailTrans(action);
                    break;
                case "clickBtnShowDetailLimitAmount":
                    clickBtnShowDetailLimitAmount(action);
                    break;
                case "clickBtnReturn_DetailLimitAmount":
                    clickBtnReturn_DetailLimitAmount(action);
                    break;
                case "clickChooseNumPage":
                    clickChooseNumPage(action);
                    break;
                default:
                    break;
            }
